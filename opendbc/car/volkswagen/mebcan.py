...
def create_acc_accel_control(packer, bus, acc_type, acc_enabled, accel, acc_control, acc_hold_type, stopping, starting, esp_hold, override, travel_assist_available):
  ...
  values = {
    ...
    "ACC_zul_Regelabw_unten":     (0.288 if acceleration > 0 else 0.144) if acc_control == ACC_CTRL_ACTIVE else 0,
    "ACC_zul_Regelabw_oben":      0.5 if acc_control == ACC_CTRL_ACTIVE and acc_control == ACC_CTRL_OVERRIDE else 0,
    "ACC_neg_Sollbeschl_Grad_02": 0.5 if acc_control == ACC_CTRL_ACTIVE else 0,
    "ACC_pos_Sollbeschl_Grad_02": 1.0 if acc_control == ACC_CTRL_ACTIVE else 0,
